...
for year, _, _, typ in courses:
    ...
    filtered = [ch for ch in choices
                if all((d, p+off) not in forbidden[year]
...
def fitness_and_violations(gene):
    ...
    vio = {'year_conflict':0,'prof_conflict':0,
           'lunch':0,'forbidden':0,'soft_days':0}
...
def rand_gene(): return [random.randrange(len(possible_slots[i])) for i in range(NUM)]

# GA 실행
pop = [(*fitness_and_violations(rand_gene()), rand_gene()) for _ in range(POP)]
best = max(pop, key=lambda x:x[0])
for _ in range(GENS):
    new = sorted(pop, key=lambda x:x[0], reverse=True)[:ELITE]
    while len(new)<POP:
        p1 = max(random.sample(pop,TOURN), key=lambda x:x[0])[2]
        p2 = max(random.sample(pop,TOURN), key=lambda x:x[0])[2]
        ...
        for ch in (c1,c2):
            for i in range(NUM):
                if random.random()<MUT: ch[i]=random.randrange(len(possible_slots[i]))
            new.append((*fitness_and_violations(ch), ch))
            if len(new)>=POP: break
    pop=new
    curr=max(pop,key=lambda x:x[0])
...
for k,v in bv.items(): print(f"  {k}: {v}")
